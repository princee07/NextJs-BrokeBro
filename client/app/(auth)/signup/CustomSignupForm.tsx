...
export default function CustomSignupForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSuccess(false);
        try {
            // Call your backend to create the user and handle referral
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-black via-gray-900 to-orange-900 animate-bgFade">
            <div className="bg-black/80 rounded-xl shadow-2xl p-8 w-full max-w-md border border-orange-500/30 relative">
                <h2 className="text-3xl font-bold text-orange-400 mb-2 text-center animate-fadeIn">Create your BrokeBro Account</h2>
                <p className="text-gray-300 mb-6 text-center animate-fadeIn delay-100">Unlock student deals, coins, and more!</p>
                <form onSubmit={handleSubmit} className="space-y-5 animate-fadeIn delay-200">
                    <div>
                        <label className="block text-orange-300 mb-1">First Name</label>
                        <input
                            type="text"
                            name="firstName"
                    ...
                    <div>
                        <label className="block text-orange-300 mb-1">Last Name</label>
                        <input
                            type="text"
                            name="lastName"
                            value={form.lastName}
                    ...
                    {/* Password field is optional for Kinde, but you can keep for UI */}
                    {/* <div>
            ...
            />
                    ...
                    {referralCode && (
                        <div className="text-xs text-amber-300 mb-2 animate-fadeIn delay-300">Referral code applied!</div>
                    )}
                    {error && <div className="text-red-400 text-sm animate-fadeIn delay-300">{error}</div>}
                    {success && <div className="text-green-400 text-sm animate-fadeIn delay-300">Signup successful! Redirecting...</div>}
                    <button
                        type="submit"
                        disabled={loading}
