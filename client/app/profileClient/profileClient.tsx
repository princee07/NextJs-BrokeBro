...
export default function ProfileClient({ user }: { user: any }) {
  ...
  // Generate secure QR code data with user token
  const qrCodeData = (() => {
    const userId = user?.id || 'unknown';
    const userName = `${user?.given_name || ''} ${user?.family_name || ''}`.trim() || 'User';
    
    // Create a secure verification URL with user ID
    const qrData = `https://brokebro.in/verify?uid=${userId}&name=${encodeURIComponent(userName)}`;
    
    console.log('Secure QR Code Data:', qrData);
    return qrData;
  })();
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-gray-900/90 rounded-2xl shadow-xl p-6">
          ...
          {/* ID Card - Optimized for QR Code Visibility */}
          <div id="id-card" ref={idCardRef} className="relative bg-orange-500 rounded-2xl p-0 border-2 border-white shadow-2xl mx-auto max-w-sm aspect-[3/5] overflow-hidden flex flex-col items-center justify-between">
            {/* BrokeBro Logo Image */}
            <img src="/assets/internpage/brokebro.png" alt="BrokeBro Logo" className="w-40 mx-auto mt-4 mb-2" style={{ objectFit: 'contain' }} />
            ...
            {/* QR Code - Simple Test */}
            <div className="flex-1 flex flex-col justify-center items-center w-full px-4 py-2">
            ...
            {/* Bottom Banner */}
            <div className="absolute bottom-0 left-0 right-0 bg-orange-500 text-black text-center py-2 font-black text-xs tracking-[0.1em] uppercase">
              STUDENT DISCOUNT
            </div>
          </div>
        </div>
